...
"""
        ...
        multi_plan = self.prepare_deployment_plan()
        nodes = multi_plan['node_instances']

        self.assertEquals(2, len(nodes))
        self.assertEquals(2, len(set(self.node_ids(nodes))))
...
"""
        multi_plan = self.prepare_deployment_plan()
        nodes = multi_plan['node_instances']
        db = self.nodes_by_name(nodes, 'db')[0]
        host = self.nodes_by_name(nodes, 'host')[0]
...
"""
        ...
        self.assertEqual(4, len(set(self.node_ids(nodes))))

        db_nodes = self.nodes_by_name(nodes, 'db')
        host_nodes = self.nodes_by_name(nodes, 'host')
        host_node_ids = self.node_ids(host_nodes)
...
"""
        ...
        self.assertEquals(5, len(set(self.node_ids(nodes))))

        host1 = self.nodes_by_name(nodes, 'host1')[0]
        host2 = self.nodes_by_name(nodes, 'host2')[0]
        db = self.nodes_by_name(nodes, 'db')[0]
        webserver = self.nodes_by_name(nodes, 'webserver')[0]
        db_dependent = self.nodes_by_name(nodes, 'db_dependent')[0]
        ...
        webserver_host_rel = self.relationships_by_target_name(
            webserver_relationships, 'host2')[0]
        webserver_db_rel = self.relationships_by_target_name(
            webserver_relationships, 'db')[0]
        self.assertEquals(host2['id'], webserver_host_rel['target_id'])
        self.assertEquals(db['id'], webserver_db_rel['target_id'])

        db_dependent_relationships = db_dependent['relationships']
        self.assertEquals(2, len(db_dependent_relationships))
        db_dependent_db_rel = self.relationships_by_target_name(
            db_dependent_relationships, 'db')[0]
        db_dependent_host_rel = self.relationships_by_target_name(
            db_dependent_relationships, 'host1')[0]
        self.assertEquals(db['id'], db_dependent_db_rel['target_id'])
        self.assertEquals(host1['id'], db_dependent_host_rel['target_id'])
...
"""
        ...
        self.assertEquals(19, len(set(self.node_ids(nodes))))

        network_1 = self.nodes_by_name(nodes, 'network')[0]
        host1_nodes = self.nodes_by_name(nodes, 'host1')
        host1_1, host1_2 = host1_nodes[:2]
        host2_nodes = self.nodes_by_name(nodes, 'host2')
        host2_1, host2_2 = host2_nodes[:2]
        host3_nodes = self.nodes_by_name(nodes, 'host3')
        host3_1, host3_2 = host3_nodes[:2]
        webserver1_nodes = self.nodes_by_name(nodes, 'webserver1')
        webserver1_1, webserver1_2 = webserver1_nodes[:2]
        webserver2_nodes = self.nodes_by_name(nodes, 'webserver2')
        webserver2_1, webserver2_2 = webserver2_nodes[:2]
        db_nodes = self.nodes_by_name(nodes, 'db')
        db_1, db_2, db_3, db_4 = db_nodes[:4]
        db_dependent_nodes = self.nodes_by_name(nodes, 'db_dependent')
        (db_dependent_1,
         db_dependent_2,
         db_dependent_3,
         db_dependent_4) = db_dependent_nodes[:4]
        ...
        self.assert_each_node_valid_hosted(webserver1_nodes, host2_nodes)
        self.assert_each_node_valid_hosted(webserver2_nodes, host2_nodes)
        self.assert_each_node_valid_hosted(db_nodes, host1_nodes)
        self.assert_each_node_valid_hosted(db_dependent_nodes, host1_nodes)
        ...
        self.assert_contained(
            webserver1_1_relationships + webserver1_2_relationships,
            self.node_ids(host2_nodes),
            'host2')
        self.assert_all_to_one(
            webserver1_1_relationships + webserver1_2_relationships,
            self.node_ids(db_nodes),
            'db')
        self.assert_contained(
            webserver2_1_relationships + webserver2_2_relationships,
            self.node_ids(host2_nodes),
            'host2')
        self.assert_all_to_all(
            ...
            'db')
        self.assert_contained(
            db_1_relationships + db_2_relationships
            + db_3_relationships + db_4_relationships,
            self.node_ids(host1_nodes),
            'host1')
        self.assert_contained(
            db_dependent_1_relationships + db_dependent_2_relationships
            + db_dependent_3_relationships + db_dependent_4_relationships,
            self.node_ids(db_nodes),
            'db')
...
"""
        self.assert_prepare_deployment_raise_exception(
            exception_types=IllegalConnectedToConnectionType)
...
"""
        self.assert_prepare_deployment_raise_exception(
            exception_types=UnsupportedRelationship)
...
node_types:
  type: {1}
""".format(instances, '{}')
        with patch('aria.deployment.relationships_graph._generate_id', lambda: randint(1, instances)):
...
"""
        plan = self.prepare_deployment_plan()
        node6_instances = [
            i for i in plan['node_instances']
            if i['node_id'] == 'node6']
        ...
        for instance in node6_instances:
            ...
            rel_groups = groupby(
                sorted(relationships, key=lambda r: r['target_name']),
...
"""
        self.assert_prepare_deployment_raise_exception(
            exception_types=UnsupportedAllToOneInGroup)
