...
class TestMultiInstanceGroups(BaseTestMultiInstance):
    ...
    def test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container(self):
        return self._test(
            groups={
                'container': {
    ...
    def test_group_with_host_db_contained_in_it_ip_and_volume2_scale_out1(self):
        self._test_modify(
    ...
    def test_group_with_host_db_contained_in_it_ip_and_volume2_scale_out2(self):
        self._test_modify(
            base=self.test_group_with_host_db_contained_in_it_ip_and_volume2,
            modified_nodes={
                'volume_scale_group': {
    ...
    def test_group_with_host_db_contained_in_it_ip_and_volume2_scale_out3(self):
        self._test_modify(
            base=self.test_group_with_host_db_contained_in_it_ip_and_volume2,
            modified_nodes={
                'db': {
    ...
    def test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container_scale_out1(self):
        self._test_modify(
            base=self.test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container,
            modified_nodes={
                'vm_with_resources': {
                    'instances': 3
                }
    ...
    def test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container_scale_in1(self):
        self._test_modify(
            base=self.test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container,
            modified_nodes={
                'vm_with_resources': {
    ...
    def test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container_scale_out2(self):
        self._test_modify(
            base=self.test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container,
            modified_nodes={
                'volume_scale_group': {
    ...
    def test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container_scale_in2(self):
        self._test_modify(
            base=self.test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container,
            modified_nodes={
                'volume_scale_group': {
    ...
    def test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container_scale_out3(self):
        self._test_modify(
            base=self.test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container,
            modified_nodes={
                'db': {
    ...
    def test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container_scale_in3(self):
        self._test_modify(
            base=self.test_group_with_host_db_contained_in_it_ip_volume_volume_c_and_container,
            modified_nodes={
                'db': {
       ...
       ``groups`` is a dict from group names to the group definitions
        ...
        a node definition has the following structure::

            {
                ...
                'HostedOn': 'host1',  # optional,
                'ConnectsTo': ['db1', 'volume1']  # optional
            }
    ...
    def _test_modify(
        ...
        modification = self.modify_multi(plan=plan, modified_nodes=modified_nodes)
        added_and_related_node_instances = modification['added_and_related']
        removed_and_related_node_instances = modification['removed_and_related']

        self._assert_modification_instances_count(
            plan=plan,
            added_and_related_node_instances=added_and_related_node_instances,
            removed_and_related_node_instances=removed_and_related_node_instances,
            expected_added_instances=expected_added_instances,
            expected_related_to_added_instances=expected_related_to_added_instances,
            expected_removed_instances=expected_removed_instances,
            expected_related_to_removed_instances=expected_related_to_removed_instances)

        (added_group_components,
         removed_group_components) = self._assert_modification_groups(
             plan=plan,
             initial_group_components=initial_group_components,
             added_and_related_node_instances=added_and_related_node_instances,
             removed_and_related_node_instances=removed_and_related_node_instances,
             expected_added_groups=expected_added_groups,
             expected_added_and_related_group_members=expected_added_and_related_group_members,
             expected_removed_groups=expected_removed_groups,
             expected_removed_and_related_group_members=expected_removed_and_related_group_members)
    ...
    def _assert_instances_count(
        ...
        expected_total = 0
        for node_id, expected_count in expected_instances.iteritems():
            expected_total += expected_count
            instances = self.nodes_by_name(nodes=node_instances, name=node_id)
    ...
    def _assert_modification_instances_count(
            ...
            expected_removed_instances,
            expected_related_to_removed_instances):
        node_instance_ids = set(i['id'] for i in plan['node_instances'])
        ...
        for instance in added_node_instances:
    ...
    def _assert_groups(
        ...
        _, contained_graph = build_previous_deployment_node_graph(
        ...
        if initial_group_components:
            _, p_contained_graph = build_previous_deployment_node_graph(
                plan_node_graph=plan_graph,
        ...
        for group_name, expected_group in expected_groups.items():
            ...
            group_instances = set(
                ...
                if not contained_graph.node[n].get('stub')
                and contained_graph.node[n]['node']['name'] == group_name)
            ...
            for group_instance in group_instances:
                ...
                contained_in_group_instances += networkx.ancestors(
                    contained_graph, group_instance)

            # build a subgraph containing only nodes that are part of any
            # instance of the current group
            group_subgraph = contained_graph.subgraph(contained_in_group_instances)

            # split subgraph into weakly connected components.
            components = networkx.weakly_connected_component_subgraphs(group_subgraph)
            ...
            if assert_new_group_instances:
                previous_group_instances = initial_group_components[group_name]['instances']
            ...
            for component in components:
                ...
                for member, count in expected_group['members'].items():
                    ...
                    component_members = [
                        ...
                        if not component.node[n].get('stub')
                        and component.node[n]['node']['name'] == member]
                ...
                component_node_members = [
                    n for n in component
                    if not component.node[n].get('stub')
                    and not component.node[n]['node'].get('group')]
                # verify group member instances count adds up
                self.assertEqual(expected_total_count, len(component_node_members))
    ...
    def _assert_relationships(
        ...
        for node_id, relationships in expected_relationships.items():
            ...
            source_instances = self.nodes_by_name(
                nodes=node_instances, name=node_id)
            ...
            for relationship in relationships:
                ...
                for source in source_instances:
                    ...
                    source_relationships = self.relationships_by_target_name(
                        source['relationships'], target_name)
                    for source_relationship in source_relationships:
                        r_graph.add_edge(source['id'], source_relationship['target_id'])
                # split graph into weakly connected components
                components = networkx.weakly_connected_component_subgraphs(r_graph)
                ...
                for component in components:
                    ...
                    for group_name in groups:
                        ancestors = group_components[group_name]['instances_and_ancestors']
                        self.assertTrue(
    ...
    def _parse_multi(self, groups, nodes):
        ...
        for node_id, node in nodes.iteritems():
            node_template = {
                'type': 'tosca.nodes.{0}'.format(node['type']),
            }
            instances = node.get('instances')
            contained_in = node.get('HostedOn')
            connected_to = node.get('ConnectsTo')
            relationships = []
            if contained_in:
        ...
        return self.prepare_deployment_plan()
