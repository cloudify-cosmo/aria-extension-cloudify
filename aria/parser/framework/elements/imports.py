...
from ...dsl_supported_versions import VERSION
...
class ImportsLoader(Element):
    schema = List(obj_type=ImportLoader)
    ...
    def _merge_into_dict(
        ...
        for key_holder, value_holder in from_dict_holder.value.iteritems():
            if key_holder.value in to_dict_holder:
                raise DSLParsingLogicException(
                    4,
                    "Import failed: Could not merge '{0}' "
                    "due to conflict on '{1}'".format(key_name, key_holder.value))
    ...
    def validate_imports_versions(
            ...
            import_url,
            parsed_imported_dsl_holder):
        _, version_value_holder = parsed_imported_dsl_holder.get_item(VERSION)
